TorrentStream.Player = function(container, conf)
{
    ...
    if(_platform == 'windows') {
        if(availablePlugin == 2 || availablePlugin == 3) {
            useQtPlugin = true;
        }
        else {
            useQtPlugin = false;
    ...
    _log("constructor: platform=" + _platform + " browser=" + _browser + " availablePlugin=" + availablePlugin + " useQtPlugin=" + useQtPlugin);

    pluginData.type = availablePlugin;
    if(useQtPlugin) {
        pluginData.qt = true;
    }
    else {
    ...
    function afterEmbed(embedObject) {
        ...
        if(!embedObject) {
            if(asyncEmbed) {
                conf.embedCallback.call(self, false, "plugin_not_installed");
                return;
            }
            else {
                throw "plugin_not_installed";
